...
class LiveCapture:
    ...
    def _process_flow_buffer(self):
        ...
        try:
            # Convert raw scapy packets to Packet objects and group into flows
            packets = [Packet(pkt) for pkt in raw_packets]
            ...
            try:
                for flow_key, flow_packets in flows.items():
                    self._analyze_flow(flow_key, flow_packets, conn)
            finally:
                conn.close()
    ...
    def _analyze_flow(self, flow_key, flow_packets, conn):
        ...
        try:
            ...
            if predicted_class != "BENIGN":
                ...
                log = Log(time_str, "alert", src_ip, dst_ip, message, proto, method)
                alert = Alert(time_str, src_ip, dst_ip, message, proto, method)
