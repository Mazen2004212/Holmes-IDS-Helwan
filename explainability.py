    ...
    try:
        ...
        return {
            ...
            "top_features": [
                ...
                for feature, value in sorted_contributions[:10]
            ],
...
def _map_stacked_to_original(model, X_scaled, shap_values_raw, feature_order):
    ...
    try:
        ...
        if isinstance(shap_values_raw, list):
            sv = np.asarray(shap_values_raw[pred])[0]
        else:
        ...
        for est in model.estimators_:
            ...
            est_shap_sum = float(np.abs(sv[idx:idx + n_outputs]).sum())
    ...
    try:
        ...
        return {
            ...
            "top_features": [
                ...
                for feature, value in clean_contributions[:10]
            ],
            "chart_base64": chart_b64,
            "glossary": FEATURE_GLOSSARY,
        }
